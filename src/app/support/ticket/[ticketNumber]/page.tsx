...
// ============================================
// Ticket Detail — عرض تفاصيل التذكرة + الردود
// support.blivoai.com/support/ticket/[ticketNumber]
// ============================================
...
type TicketSummary = { id: string; ticketNumber: string; subject: string; status: string }
...
export default function TicketDetailPage({ params }: { params: Promise<{ ticketNumber: string }> }) {
  ...
  const loadTicket = useCallback(async () => {
    ...
    try {
      // البحث عن التذكرة بالبريد الأول
      const listRes = await fetch(`/api/support/tickets?email=${encodeURIComponent(email)}`)
      const listData = await listRes.json()
      const found: TicketSummary | undefined = (listData.tickets || []).find(
        (t: TicketSummary) => t.ticketNumber === ticketNumber
      )
      if (!found) {
        setError("التذكرة غير موجودة أو البريد لا يتطابق")
        setLoading(false)
        return
      }
      // تحميل التفاصيل
      ...
      if (detailRes.ok) {
        ...
        setError("لا يمكن تحميل التذكرة")
      }
    } catch {
  ...
  const handleReply = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setReplyContent("")
      setSuccessMsg("تم إرسال ردك بنجاح")
      loadTicket()
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white">
        ...
        <div className="max-w-md mx-auto text-center py-16 px-4">
          ...
          <p className="text-slate-400 mb-6">أدخل بريدك الإلكتروني لعرض التذكرة <span className="font-mono text-blue-400" dir="ltr">{ticketNumber}</span></p>
          <form onSubmit={(e) => {
