...
export default function LoginPage() {
  ...
  const handleGoogleClick = async () => {
    ...
    try {
      ...
      if (!clientId) {
        setError(isRTL ? "Google غير متوفر حاليا" : "Google sign-in not available")
            ...
            if (response.credential) {
              try {
                ...
                if (r.ok && d.user) {
                  ...
                  setError(d.error || (isRTL ? "فشل تسجيل الدخول" : "Login failed"))
                  setGoogleLoading(false)
                }
              } catch {
                setError(isRTL ? "خطأ في الاتصال" : "Connection error")
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center px-4 py-8" dir={isRTL ? "rtl" : "ltr"}>
      <div className="w-full max-w-md space-y-6">
          ...
          <CardContent className="p-4 sm:p-6">
            <form onSubmit={handleSubmit} noValidate className="space-y-4">
              <div className="space-y-2">
                ...
                <Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="ahmed@example.com" className="bg-slate-800 border-slate-700 text-white placeholder:text-slate-500 h-11" dir="ltr" autoFocus />
              </div>
              <div className="space-y-2">
                <Label className="text-slate-300">{t("auth.login.password", language)}</Label>
                <Input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder={t("auth.login.password", language)} className="bg-slate-800 border-slate-700 text-white placeholder:text-slate-500 h-11" dir="ltr" />
              </div>
              <Button type="submit" className="w-full bg-emerald-600 hover:bg-emerald-700 text-white py-3 h-11" disabled={loading}>
        ...
        <div className="text-center">
          <button onClick={() => router.push(`/${language}/signup`)} className="text-emerald-500 text-sm hover:text-emerald-400 transition-colors cursor-pointer">
            {t("auth.login.noAccount", language)} {t("auth.login.signupLink", language)}
          </button>
        </div>
        <div className="text-center">
          <button onClick={() => router.push(`/${language}`)} className="text-slate-500 text-sm hover:text-slate-300 transition-colors cursor-pointer">
