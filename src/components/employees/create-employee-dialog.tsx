...
// يدعم: اسم + مسمى وظيفي + تخصص (دروب داون للتخصصات الموجودة + إدخال حر لتخصص جديد) + قسم
// كل موظف تخصص واحد فقط — ما يقدرش يتجاوزه أبداً
// التخصصات مش محدودة — المستخدم ينشأ أي تخصص يريده
// ============================================
...
interface CreateEmployeeDialogProps {
  ...
  employees?: IEmployee[]  // ← لنستخرج التخصصات الموجودة
}

// اقتراحات مسميات وظيفية — فقط اقتراحات عشان تساعد، مش قائمة محددة
...
// أمثلة تخصصات للاقتراحات المبدئية (لو ما في تخصصات موجودة بالشركة)
const DEFAULT_SPECIALIZATION_HINTS: Record<string, string[]> = {
  ar: [
    "إدارة متجر إلكتروني",
    "تعبئة بيانات Excel",
    "محاسبة مالية",
...
export function CreateEmployeeDialog({
  ...
  // استخراج التخصصات الموجودة بالشركة (فريدة)
  ...
  // لو ما في تخصصات موجودة → خلّي mode "new" مباشرة
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="bg-slate-900 border-slate-800 text-white max-w-md" dir="ltr">
        <DialogHeader>
        ...
        {step === "name" ? (
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              <Label className="text-slate-300">{t("createEmployee.name", language)}</Label>
              <Input
                value={name}
                onChange={(e) => setName(e.target.value)}
                onKeyDown={(e) => e.key === "Enter" && handleNameSubmit()}
                placeholder={t("createEmployee.namePlaceholder", language)}
                className="bg-slate-800 border-slate-700 text-white placeholder:text-slate-500 text-lg py-3"
                autoFocus
              />
            </div>
            <Button
          ...
          <div className="space-y-4 py-2">
            {/* المسمى الوظيفي */}
            <div className="space-y-2">
              <Label className="text-slate-300">{t("createEmployee.role", language)}</Label>
              <Input
                value={role}
            ...
            <div className="space-y-2">
              ...
              {effectiveMode === "select" && existingSpecializations.length > 0 ? (
                /* دروب داون — التخصصات الموجودة بالشركة */
                <select
                  value={specialization}
                  onChange={(e) => setSpecialization(e.target.value)}
                  className="w-full bg-slate-800 border border-slate-700 text-slate-300 text-sm rounded-lg px-3 py-2.5"
                >
                  <option value="">
              ...
              {/* اقتراحات سريعة — بس لو في mode "new" */}
              {effectiveMode === "new" && (
                <div className="flex flex-wrap gap-1.5">
                  {(DEFAULT_SPECIALIZATION_HINTS[language] || DEFAULT_SPECIALIZATION_HINTS.en).map((spec) => (
                    <button
                      key={spec}
            ...
            {/* وصف الدور */}
            <div className="space-y-2">
              <Label className="text-slate-300">{t("createEmployee.roleDescription", language)}</Label>
              <Textarea
