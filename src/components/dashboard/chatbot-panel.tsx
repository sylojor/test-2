...
const CHATBOT_STORAGE_KEY = "blivoai_chatbot_conversations"
const CHATBOT_ACTIVE_KEY = "blivoai_chatbot_active_conv"

// --- Helper: serialize conversations for localStorage ---
...
// --- Helper: deserialize conversations from localStorage ---
function deserializeConversations(data: string): Conversation[] {
  try {
    ...
    return parsed.map((c: Record<string, unknown>) => ({
      ...
      messages: (c.messages as Array<Record<string, unknown>>).map((m: Record<string, unknown>) => ({
        ...m,
...
// --- Helper: generate a smart title from the first message ---
function generateTitle(firstMessage: string, language: string): string {
  const trimmed = firstMessage.trim()
  if (!trimmed) return language === "ar" ? "محادثة جديدة" : "New Chat"
  // أخذ أول 40 حرف كعنوان
  const maxLen = 40
...
export function ChatbotPanel() {
  ...
  const [conversations, setConversations] = useState<Conversation[]>(() => {
    ...
    try {
      ...
      if (saved) return deserializeConversations(saved)
    } catch {}
    return []
  })
  const [activeConversation, setActiveConversation] = useState<string | null>(() => {
    ...
    try {
      return localStorage.getItem(CHATBOT_ACTIVE_KEY) || null
    } catch {}
    return null
  })
  ...
  // --- حفظ المحادثات في localStorage كل ما تتغير ---
  useEffect(() => {
    if (typeof window === "undefined") return
    try {
      localStorage.setItem(CHATBOT_STORAGE_KEY, serializeConversations(conversations))
    } catch {}
  }, [conversations])
  ...
  useEffect(() => {
    ...
    try {
      if (activeConversation) {
        localStorage.setItem(CHATBOT_ACTIVE_KEY, activeConversation)
      } else {
        localStorage.removeItem(CHATBOT_ACTIVE_KEY)
      }
    } catch {}
  }, [activeConversation])
  ...
  const sendMessage = async () => {
    ...
    // Update conversation with user message + عنوان ذكي
    setConversations(prev => {
      const existing = prev.find(c => c.id === convId)
      if (existing) {
        const isFirstMessage = existing.messages.length === 0
        const updated = {
          ...existing,
          messages: [...existing.messages, userMessage],
          title: isFirstMessage
            ? generateTitle(messageText, language)
            : existing.title,
        }
        ...
        return [{
          id: convId,
          title: generateTitle(messageText, language),
    ...
    try {
      // Auth handled via HttpOnly cookie (oec_token) — sent automatically by browser
