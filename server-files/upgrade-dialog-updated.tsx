...
// ALL users can pay with Visa like any online purchase
// Money goes to the owner's Dodo account automatically
// The paying user only sees the checkout page — no config details
//
...
import { CheckCircle2, Users, ArrowUpRight, Loader2, ExternalLink, Trash2, RefreshCw } from "lucide-react"
...
export function UpgradeDialog({
  ...
  // ALL users can pay — money goes to the company owner's Dodo account
  const handlePayAndUpgrade = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.checkoutUrl) {
          // Redirect user to Dodo checkout — they pay with Visa like any online purchase
          window.location.href = data.checkoutUrl
        } else {
          toast.error(isArabic ? "لم يتم إنشاء رابط الدفع" : "Payment checkout URL not generated")
        }
      } else if (res.status === 503) {
        // Payment not available yet — show generic message without config details
        setPaymentError(isArabic 
          ? "خدمة الدفع غير متاحة حالياً. يمكنك حذف أو استبدال موظف موجود لتفريغ مكان." 
          : "Payment service is currently unavailable. You can delete or replace an existing employee to free up a slot."
        )
        toast.error(isArabic ? "خدمة الدفع غير متاحة حالياً" : "Payment service is currently unavailable")
      } else {
  ...
  // ============================================
  // ALL USERS: Full upgrade + payment flow
  // They just pick a plan and pay with Visa
  // Money goes to the owner's Dodo account automatically
  // ============================================
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        {/* Payment unavailable — generic message without config details */}
        {paymentError && (
          <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700/30 rounded-lg p-3 flex items-start gap-2">
            <ArrowUpRight className="w-4 h-4 text-amber-600 dark:text-amber-400 mt-0.5" />
