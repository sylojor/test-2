...
// OWNER: Shows available plans → redirected to Dodo Payments checkout
// NON-OWNER: Shows "Only the owner can upgrade" message
...
import { CheckCircle2, Users, ArrowUpRight, Loader2, ExternalLink, AlertTriangle, Trash2, RefreshCw, ShieldAlert } from "lucide-react"
...
export function UpgradeDialog({
  ...
  const handlePayAndUpgrade = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Not owner — payment forbidden
        toast.error(isArabic ? "بس صاحب الشركة يقدر يدفع ويرقّي الاشتراك" : "Only the company owner can pay and upgrade the subscription")
      } else {
        const errData = await res.json()
  ...
  if (!isOwner) {
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent
          className="bg-card border-border text-foreground max-w-md max-h-[90vh] overflow-y-auto scrollbar-custom"
        ...
        >
          <DialogHeader>
            ...
            <DialogDescription className="text-muted-foreground">
              {isArabic
                ? `خطتك الحالية (${SUBSCRIPTION_PLANS[currentPlan].nameAr}) محدودة بـ ${SUBSCRIPTION_PLANS[currentPlan].maxEmployees} موظفين فقط. بس صاحب الشركة يقدر يرقّي الاشتراك ويدفع.`
                : `Your current plan (${SUBSCRIPTION_PLANS[currentPlan].name}) is limited to ${SUBSCRIPTION_PLANS[currentPlan].maxEmployees} employees. Only the company owner can upgrade and pay for a higher plan.`
              }
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700/30 rounded-lg p-4 flex items-start gap-3">
            ...
            <div className="text-sm">
              <p className="text-amber-700 dark:text-amber-400 font-medium">
                {isArabic ? "بس الأونر يقدر يدفع ويرقّي" : "Only the owner can pay & upgrade"}
              </p>
              <p className="text-amber-600/70 dark:text-amber-400/70 mt-1.5">
                {isArabic
                  ? "ترقية الاشتراك والدفع متاحة فقط لصاحب الشركة (الأونر). تواصل مع الأونر عشان يرقّي، أو احذف/استبدل موظف موجود عشان تفرغ مكان."
                  : "Subscription upgrade and payment is only available to the company owner. Contact the owner to upgrade, or delete/replace an existing employee to free up a slot."
                }
              </p>
            </div>
          </div>
  ...
  // ============================================
  // OWNER: Full upgrade + payment flow
  // ============================================
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
